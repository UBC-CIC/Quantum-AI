  ...
  return (
    <div className="ml-16 mb-6 mr-16">
      <div className="flex flex-row flex-start">
        ...
        >
          {/* Use ReactMarkdown for parsing the message content */}
          {typeof message === "string" ? (
            <ReactMarkdown
              components={renderers}
              remarkPlugins={[remarkGfm]}
              rehypePlugins={[rehypeRaw]} // Optional: allows HTML rendering
            >
              {message}
            </ReactMarkdown>
          ) : (
            <div className="text-red-500">Invalid message content</div>
          )}
        </div>
        <button
      ...
      {showFeedbackForm && !feedbackSubmitted && (
        <div className="mt-4 ml-14 p-4 bg-gray-100 rounded-md">
          <h3 className="text-lg font-semibold mb-2 text-black">
            How was your experience with Quantum AI?
          </h3>
          <div className="mb-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
          ...
          <button
            ...
            onMouseEnter={(e) =>
              (e.target.style.backgroundColor = "#0d3140")
            }
            onMouseLeave={(e) =>
              (e.target.style.backgroundColor = "#114153")
            }
          >
            Submit Feedback
          </button>
